 ...
 */
package pdfreader.infuturetech.com.jiopdfviewerlite.pdflite.pdfbox.pdmodel.graphics.shading;
...
abstract class Patch
{
     ...
     */
    protected abstract Point[] getFlag1Edge();
     ...
     * @return implicit control points
     */
    protected abstract Point[] getFlag2Edge();
     ...
     * @return implicit control points
     */
    protected abstract Point[] getFlag3Edge();
     ...
     */
    protected double getLen(Point ps, Point pe)
    {
        double x = pe.x - ps.x;
        double y = pe.y - ps.y;
     ...
     */
    protected boolean isEdgeALine(Point[] ctl)
    {
     ...
     */
    protected double edgeEquationValue(Point p, Point p1, Point p2)
    {
    ...
    protected List<ShadedTriangle> getShadedTriangles(CoordinateColorPair[][] patchCC)
    {
        ...
        for (int i = 1; i < szV; i++)
        {
            for (int j = 1; j < szU; j++)
            {
                Point p0 = patchCC[i - 1][j - 1].coordinate, p1 = patchCC[i - 1][j].coordinate, p2 = patchCC[i][j].coordinate,
    ...
    private boolean overlaps(Point p0, Point p1)
    {
