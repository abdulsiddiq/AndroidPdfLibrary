 ...
 */
package pdfreader.infuturetech.com.jiopdfviewerlite.pdflite.pdfbox.io;
...
public class ScratchFile implements Closeable
{
    ...
    public static ScratchFile getMainMemoryOnlyInstance()
    {
        try
        {
//            use  temp file only for less usage of memory in mobile
            return new ScratchFile(MemoryUsageSetting.setupTempFileOnly());
//            return new ScratchFile(MemoryUsageSetting.setupMainMemoryOnly());
        }
        catch (IOException ioe)
        {
            // cannot happen for main memory setup
            Log.e("scratchFile","Unexpected exception occurred creating main memory scratch file instance: " + ioe.getMessage() );
            return null;
        }
    }
            ...
            if (useScratchFile)
            {
                ...
                if ( raf == null )
                {
                    ...
                    catch (IOException e)
                    {
                        if (!file.delete())
                        {
                            Log.e("scratchFile","Error deleting scratch file: " + file.getAbsolutePath());
                        }
                        throw e;
                    }
                }
